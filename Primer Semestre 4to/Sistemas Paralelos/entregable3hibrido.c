...
int main(int argc, char* argv[]){
	...
	if ((argc != 3) || ((n = atoi(argv[1])) <= 0) || ((threads = atoi(argv[2])) < 2) ) {
	...
	#pragma omp parallel
	{
		...
		//sadfñlajsbnfpkasjfbaspkfhjabsfñkashjbfasñikjfbhasf fijate que el num_threads capaz lo podemos sacar tirando la línea omp_set_num_threads(threads);
		#pragma omp parallel for num_threads(threads) reduction(+:sumatoriaR1, sumatoriaR2) schedule(static)
