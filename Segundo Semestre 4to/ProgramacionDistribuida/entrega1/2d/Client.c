...
int main(int argc, char *argv[])
{
    ...
    /* Cargo el buffer en todas sus posiciones con el caracter 'A'.
    En la ultima posicion indico el fin del string con 'Z' */
    for(int i = 0; i < bufferSize - 1; i++)
        buffer[i] = 'A';
    buffer[bufferSize - 1] = 'Z';

    // Se convierte el tamanio al orden de bytes del sistema
    int messageSize = htonl(bufferSize);
